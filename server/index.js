...
async function initializeData() {
    ...
    const studentExists = collections.some((collection) => collection.name === studentCollection);
    const adminExists = collections.some((collection) => collection.name === adminCollection);
    const aspirationExists = collections.some((collection) => collection.name === aspirationCollection);
...
async function signIn(email, password) {
    ...
    if (email.includes("students")) {
        ...
        if (studentCheck.length == 0) {
            ...
            const dataToInsert = [{ _id: email, fullname: fullname, username: email.split("@")[0], datejoined: DATE, password: password, active: true, role: "Mahasiswa" }];
            const result = await student.insertMany(dataToInsert);
            return true;
        } else {
            const credentialCheck = await student.find({ _id: email, password: password }).toArray();
        ...
        if (adminCheck.length == 0) {
            ...
            const dataToInsert = [{ _id: email, fullname: fullname, username: email.split("@")[0], datejoined: DATE, password: password, active: true, role: "Admin" }];
            const result = await admin.insertMany(dataToInsert);
            return true;
        } else {
            const credentialCheck = await admin.find({ _id: email, password: password }).toArray();
...
async function saveForm(title, tipe, to, message, tags, anonym) {
    ...
    const dataToInsert = [{ _id: formId, fullname: currentName, type: tipe, title: title, datecreated: currentTime, to: to, message: message, tags: tags, status: status, reply: 0, like: 0, dislike: 0 }];
