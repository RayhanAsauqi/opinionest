...
export default function InputTo(props: TipeProps) {
	...
	const [selectedOption, setSelectedOption] = useState("Choose target");
		...
		<div className="relative w-full flex flex-col">
			...
			{isOpen && (
				<div className="w-full absolute mt-[100px] border border-[#D5D5D5] bg-white rounded-[10px] z-20 py-[10px] px-[10px] divide-y divide-[#D5D5D5]">
					<ButtonDown title="Direktorat Keuangan" type="text" className="text-[16px]" onClick={() => handleOptionClick("Direktorat Keuangan")} />
					<ButtonDown title="DKI" type="text" className="text-[16px]" onClick={() => handleOptionClick("DKI")} />
					<ButtonDown title="Serikat Mahasiswa" type="text" className="text-[16px]" onClick={() => handleOptionClick("Serikat Mahasiswa")} />
					<ButtonDown title="Fakultas Ilmu Rekayasa" type="text" className="text-[16px]" onClick={() => handleOptionClick("Fakultas Ilmu Rekayasa")} />
				</div>
			)}
		</div>
	);
