...
export default function CardReport(props: CardReportProps) {
  ...
  const agreeSubmit = async (e: { preventDefault: () => void }) => {
    ...
    const postLike = await axios.post("http://localhost:3001/postlike", {
      formId,
      agree,
    });
  };
  ...
  const disagreeSubmit = async (e: { preventDefault: () => void }) => {
    ...
    const postDislike = await axios.post("http://localhost:3001/postdislike", {
      formId,
      disagree,
    });
  };
